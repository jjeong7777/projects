...
def generate_launch_description():
    ...
    world = os.path.join(
        turtlebot3_multi_robot, "worlds", "real_world"  ##@@@경로수정
    )
...
#*****************바뀐거
# 원기둥 스폰 코드 추가
    # Add random cylinder spawn
    for i in range(3):  # Spawn 3 random cylinders
        random_x = random.uniform(-5, 5)  # Define the range for random x
        random_y = random.uniform(-5, 5)  # Define the range for random y
        cylinder_name = f"cylinder_{i}"

        spawn_cylinder = Node(
            package="gazebo_ros",
            executable="spawn_entity.py",
            arguments=[
                "-file", cylinder_model_path,
