...
def generate_launch_description():
    ...
    world = os.path.join(
        turtlebot3_multi_robot, "worlds", "world_final"  ##@@@경로수정 world_final이 찐최종
    )
    ...
    # 랜덤 좌표 생성
    random_x = random.uniform(-7, 5.5)  # X 좌표 범위 (-5 ~ 5)
    random_y = random.uniform(-6, 5.5)  # Y 좌표 범위 (-5 ~ 5)
    random_z = random.uniform(0.5, 1)  # Z 좌표 범위 (0.5 ~ 1)

    # 스폰 노드 생성
    cylinder_urdf = os.path.join(
        turtlebot3_multi_robot, "urdf", "cylinder.urdf"  # 원기둥 URDF 파일 경로
    )
    spawn_cylinder = Node(
        package="gazebo_ros",
        executable="spawn_entity.py",
        arguments=[
            ...
            "-entity", "cylinder",
             "-x", str(random_x),
            "-y", str(random_y),
            "-z", str(random_z),  # 스폰 좌표
        ],
        output="screen",
    )
    ld.add_action(spawn_cylinder)
